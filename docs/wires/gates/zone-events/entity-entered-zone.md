---
icon: material/text-box-plus-outline
tags:
    - Wires
    - Gates
    - Zones
...
When a brick grid or physics object enters the zone referenced by
the {{var_display("Zone Reference","special")}} input,
the gate will output an {{var_display("Exec","exec")}} signal and a
reference of the brick grid or physics object that entered the zone
...
reference for every brick grid or physics object that enters the
zone.
